...
class DeepPhyLSTM:
    ...
    def CNN_model(self, X):
        model = Sequential()
        model.add(Conv1D(64, 40, strides=1, padding='same', use_bias=True, input_shape=(None, 1)))
        model.add(Activation('relu'))
        model.add(Conv1D(64, 40, strides=1, padding='same', use_bias=True))
        model.add(Activation('relu'))
        model.add(Conv1D(64, 40, strides=1, padding='same', use_bias=True))
        model.add(Activation('relu'))
        model.add(Conv1D(64, 40, strides=1, padding='same', use_bias=True))
...
with tf.device('/device:GPU:0'):
    ...
    # Training
    model = DeepPhyLSTM(eta_train, eta_t_train, g_train, ag_train, lift_train, Phi_t)
    ...
    for jj in range(n_p):
        ...
        eta, eta_t, eta_dot, g = model.predict(X_pred[ind_str:ind_end])
    ...
    for ii in range(len(y_pred)):
        ...
        plt.figure()
        plt.plot(g_pred_ref[ii], label='True')
